...
class TreeList{
    ...
    createHashLists(list){
        ...
        var hashedList=cthis.reateHashedList(list);
        ...
        if(this.createSubArray(notHashedList[i], notHashedList).length==1){
            hashedValue = this.calculateDirectoryHash(notHashedList[i],this.createSubArray(notHashedList[i], hashedList));
    ...
    createHashedList(list){
        ...
        for (var path of list) {
        result.push(path + ":h:" + merkle.fileHashSync(path));
        }
