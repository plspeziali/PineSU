...
class TreeList {
    ...
    createHashTree(list) {
        ...
        const notHashedList = this.createDirectoryList(list);
        const hashedList = this.createHashedList(list);
    ...
    createDirectoryList(list) {
        ...
        directoryList = directoryList.filter(function (e) {
            return e
        });
    ...
    createCompSubArray(substring, array) {
        const result = Array();
        for (let el of array) {
            if (typeof el !== "undefined" && !el.includes(substring) && el != substring) {
                result.push(el);
            }
        }
        return result;
    ...
    calculateDirectoryHash(dirpath, contentslist) {
        ...
        const root = this.calculateTree(lowerLevel);
