...
class TreeList{
    ...
    combineHash(day, hour, minute, second, hash){
        return '' + crypto
                    .createHash("sha256")
                    .update(day.toString() + hour.toString() + minute.toString() + second.toString() + hash.toString())
                    .digest("hex");      
    }
    ...
    calculateDirectoryHash(dirpath,contentslist){
        ...
        var root = this.calculateTree(lowerLevel);
