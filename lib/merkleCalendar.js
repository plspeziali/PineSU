...
class MerkleCalendar{
    ...
    generateProof(node){
        ...
        var proof = this.calculateProof(leaf.getHash());
        return proof;
    }

    checkProof(node, proofTree){
        var result = validateProof(proofTree.monthProof, node.getHash(), proofTree.monthHash);
        node = node.getParent();
