...
class MerkleCalendar{
    ...
    generateProof(leaf){
        ...
        // IDEA: Farne tre separati, uno per il mese, poi per l'anno poi per il C/O
        // Così come è stato scritto NON PUò funzionare

        var proofTree = {};
        var actualNode = leaf;
        do {
            var parent = actualNode.getParent();
            var hashes = parent.getChildrenHashes();
            this.calculateHash(hashes);
            var proof = this.calculateProof(leaf.getHash())[0];
            proofTree.psuh(proof);
            actualNode = parent;
        }
        while(parent.getName() != "Open" && parent.getName() != "Closed");
