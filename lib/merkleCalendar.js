...
class MerkleCalendar{
    ...
    getBSPRoot(hash){
        ...
        return this.calculateHash(yearsHash);
        /*let newRoot = this.calculateHash(yearsHash);
        var roots = new Array();
        roots.push(newRoot);
        if(closed){
            let openHash = this.#open.getHash();
            if(openHash != null){
                roots.push(openHash);
            }
        }else{
            let closedHash = this.#closed.getHash();
            if(closedHash != null){
                roots.push(closedHash);
            }
        };
        return this.calculateHash(roots);*/
    }
