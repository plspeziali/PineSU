...
const MerkleCalendar = require('merkle-calendar');
...
module.exports = {
    ...
    loadTree: () => {
        ...
        let openFile, closedFile;
        if (fs.existsSync(__dirname + '/../merkles/open.json')) {
            openFile = fs.readFileSync(__dirname + '/../merkles/open.json', 'utf8');
            const openT = JSON.parse(openFile);
            for (y of openT) {
                for (m of y.children) {
                    for (l of m.children) {
                        mc.addRegistrationNC(l.name, l.hash, l.year, l.month, l.day, l.hour, l.minute, false, m.hash, y.hash, l.storageGroup);
                    }
                }
            }
        }
        if (fs.existsSync(__dirname + '/../merkles/closed.json')) {
            ...
            const closedT = JSON.parse(closedFile);
            for (y of closedT) {
                for (m of y.children) {
                    for (l of m.children) {
                        mc.addRegistrationNC(l.name, l.hash, l.year, l.month, l.day, l.hour, l.minute, true, m.hash, y.hash, l.storageGroup);
                    }
                }
            }
        }
