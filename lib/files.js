...
module.exports = {
    ...
    createSGTrees: (sg) => {
        ...
        for (el of open) {
            el.proofSG = treelist.getProof(el.hash);
        }
        let closedRoot = null;
    ...
    savePineSUJSON: async (obj) => {
        ...
        if (fs.existsSync(process.cwd().replace(/\\/g, "/") + "/.pinesu.json")) {
            ...
            fs.writeFileSync(process.cwd().replace(/\\/g, "/") + "/.pinesuhistory/pinesu" + content.date + "_" + content.hash + ".json", contentJSON);
        }
    ...
    getFilelist: (dir, filelist) => {
        ...
        const forbiddenFiles = [".git", ".pinesuhistory", ".pinesu.json", ".registration.json", ".gitignore", ".gitkeep"];
    ...
    createRegistration: (el) => {
        ...
        if (fs.existsSync(process.cwd().replace(/\\/g, "/") + "/.registration.json")) {
            ...
            module.exports.createPineSUOld();
            const contentJSON = JSON.stringify(content);
            fs.writeFileSync(process.cwd().replace(/\\/g, "/") + "/.pinesuhistory/registration" + content.date + "_" + content.root + ".json", contentJSON);
        }
        ...
        if (fs.existsSync(process.cwd().replace(/\\/g, "/") + "/.pinesu.json")) {
            const content = module.exports.readPineSUFile();
            content.header.prevmkcalroot = content.header.merkleroot;
            content.header.prevbcregnumber = content.offhash.bcregnumber;
            content.header.prevbcregtime = content.offhash.bcregtime;
            content.header.prevclosed = content.offhash.closed;
            content.header.merkleroot = el.mkcalroot;

            content.offhash.bcregnumber = content.offhash.bcregnumber+1;
            content.offhash.bcregtime = el.mkcaltimestamp;
            module.exports.savePineSUJSON(content);
        }
    },
    ...
    saveTree: (mc) => {
        try {
            ...
            fs.writeFileSync(__dirname + '/../merkleCalendar.json', JSON.stringify(mcString));
        } catch (err) {
            console.log('There has been an error parsing your JSON.')
            console.log(err);
        }
