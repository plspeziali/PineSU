...
module.exports = {
  ...
  loadTree: () => {
    ...
    try{
        openFile = fs.readFileSync(__dirname+'/open.json', 'utf8');
        closedFile = fs.readFileSync(__dirname+'/closed.json', 'utf8');
        mc.loadTree(JSON.parse(openFile),JSON.parse(closedFile))
    } catch(err){
        if(err.code === 'ENOENT') {
            mc = new MerkleCalendar();
        };
        return mc;
    }
  },
  ...
  saveTree: (mc) => {
    try {
      [openS, closedS] = mc.getTree();
      fs.writeFileSync(__dirname+'/open.json', JSON.stringify(openS));
      fs.writeFileSync(__dirname+'/closed.json', JSON.stringify(closedS));
    } catch (err) {
      console.log('There has been an error parsing your JSON.')
      console.log(err);
    }
