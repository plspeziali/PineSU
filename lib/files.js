...
module.exports = {
  ...
  addToUser: (_userhash, _project, _suinfo) => {
    var hash =  crypto.createHash('md5').update(JSON.stringify(_suinfo), 'utf8').digest('hex');
    var data = fs.readFileSync(path.resolve(__dirname+'/..', 'config.json'));
    try {
      var myObj = JSON.parse(data);
      ...
      if(!sulist.includes(_project+":"+hash)){
        sulist.push(_project+":"+hash);
      }
  ...
  writeHashes: (list) => {
    ...
    if(list.length == 1){
      ...
      res.push({
        hash: treelist.calculateTree(hashlist),
        name: today,
        proof: "null"
      });
    }
    ...
    data = data.push(JSON.stringify(res));

    fs.writeFileSync(path.resolve(__dirname+'/..', 'registrations.json'), data);
  ...
  createZIP: (list, json) => {
    ...
    for(var el of list){
      ...
      if(path.includes("/")){
        ...
        var charnum = (arr[arr.length-1]+'').length+2;
        zip.addLocalFile(process.cwd().replace(/\\/g, "/")+"/"+path,path.slice(0,-charnum));
      } else {
